...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
    ...
    return {
        title: `${tip.title} - ZROOM AI Tips`,
        description: tip.summary,
        keywords: `AI팁, ${tip.title}, AI가이드, 프롬프트`,
        openGraph: {
            title: `${tip.title} - ZROOM AI Tips`,
            description: tip.summary ?? undefined,
            url: `https://zroom.io/ai/tips/${tip.id}`,
            siteName: 'ZROOM',
        ...
        alternates: {
            canonical: `https://zroom.io/ai/tips/${tip.id}`,
        },
    };
}
...
export default async function TipDetailPage({ params }: PageProps) {
    ...
    const relatedTips = await fetchRelatedTips('ai', id, 2);

    return <TipDetail tip={tip} relatedTips={relatedTips} theme="ai" />;
}
